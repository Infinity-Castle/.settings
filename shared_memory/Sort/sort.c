...
// __intptr_t is a type defined in the C standard library for representing a data pointer that can hold a pointer value or an integer value. It is designed to be large enough to hold the bit pattern of any valid pointer on the system.
// The __intptr_t type is defined in the <stdint.h> header file, and it is guaranteed to be a signed integer type that is at least as large as a pointer. The exact size of __intptr_t depends on the system and architecture, but it is typically the same size as the pointer size on that system.